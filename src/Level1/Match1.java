...
public class Match1 {
    ...
    // 점수를 계산하는 객체
    private final ScoreCalculator1 scoreCalculator;
    // 사용자에게 정보를 출력하는 객체
    private final StadiumDisplay1 display;


    // 생성자: 새로운 게임을 시작하기 위한 초기화
    public Match1() {
        this.targetNumbers = NumberGenerator1.generateTargetNumbers(); // 타겟 숫자 생성
        this.scoreCalculator = new ScoreCalculator1(targetNumbers); // 점수 계산기 초기화
        this.display = new StadiumDisplay1(); // 디스플레이 초기화
    }
    ...
    public void start() {
        ...
        do {
            // 게임 시작 메뉴 출력
            System.out.println("-------------------");
            System.out.println("1. 게임시작 2. 종료하기");
            System.out.println("-------------------");
            ...
            if (choose == 1) {
                ...
                while (!imTheWinner) {
                    ...
                    // 재시작 시 새로운 타겟 숫자 생성
                    if (playAgain) {
                    targetNumbers = NumberGenerator1.generateTargetNumbers(); // 새로운 타겟 숫자 생성
